...
if st.button('Show Results'):
  ...
  for i in range(1,n+1):           
   ...
   AVG=R["WHP"].astype('float').mean()           
   AVG1=R["SEP_Pressure"].astype('float').mean()           
   AVG2=R["SEP_Temperature"].astype('float').mean()           
   AVG3=R["FLP"].astype('float').mean()     
   AVG4=R["FLT"].astype('float').mean()          
   AVG5=R["Gas_Rate"].astype('float').mean()           
   AVG6=R["Water"].astype('float').mean()           
   AVG7=R["Condensate"].astype('float').mean()           
   AVG8=R["GOR"].astype('float').mean()           
   AVG9=R["API"].astype('float').mean()           
   AVG10=R["BS&W"].astype('float').mean()   
